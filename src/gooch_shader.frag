...
vec4 getPhongLighting(vec3 vpos, vec3 n)
{
	...
	const float Kd = 1.0;
	const float Ks = 0.9;
	vec4 blue = vec4(0.62,0.58,1.0,1.0);
	vec4 pink = vec4(1.0,0.30,0.3,1.0);
	vec4 kCool = pink;
	vec4 kWarm = blue;
	...
	vec4 Cgooch = mix(diffuse,kWarm,kCool);
	vec4 fColor = Ka*ambient + specularcolor*Ks*specular + kd*Cgooch;
...
void main()
{
	...
	vec4 fcolor = getPhongLighting(vPos.xyz, normalize(normal+normal_bias));
