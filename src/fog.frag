...
void main()
{
	...
	vec4 fogColor = vec4(0.7,0.7,0.7,0.7);
	float intensity = 0.005;
	float fogFactor = 1.0 - exp(-intensity*intensity*abs(vPos.z)*abs(vPos.z)*1.442695);
	float fog = clamp(fogFactor, 0.0, 1.0);

	if (has_fog) {
		gl_FragData[0] = fog * fogColor + shadow_ambient * shadow_coef * fcolor + (1.0 - shadow_ambient) * color_tex;
	}
	else{
